...
function Track({ song, artist }) {
  ...
  const [play, setPlay] = useState(false); // flag to handle the click
  const { currentSong, setNewSong } = useContext(songContext)

  /**
   * Define behavior when pause or play button has been clicked
   */
  const handler = () => {
    // if it's the same song 
    if (currentSong.src === song.source) {
      if (play) {
        currentSong.pause();
      } else {
        currentSong.play()
  ...
  useEffect(() => {
    currentSong.addEventListener('ended', () => setPlay(false));
    return () => {
      currentSong.removeEventListener('ended', () => setPlay(false));
    };
  }, []);
  ...
  return (
    <tr className="w-full min-h-[30px] md:w-full">
      ...
      <td className="p-2">
        {
          // if current song and prop has the same src [and it's playing] show pause icon, otherwise show pause icon
          currentSong.src === song.source && play ? pauseIcon : playIcon
        }
      </td>
